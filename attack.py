...
class ANFIS:
    ...
    def _anfis_loss(self, params):
        ...
        for j in range(self.num_features):
            ...
            for k in range(self.num_rules):
                ...
                b = params[param_index + 1]
                c = params[param_index + 2]
                mfs.append(lambda x, a=a, b=b, c=c: gbell_mf(x, a, b, c))
                param_index += 3
            new_mfs.append(mfs)
    ...
    def train(self, X_train, y_train):
        ...
        for j in range(self.num_features):
            mfs = []
            for k in range(self.num_rules):
                a = self.optimized_params[param_index]
                b = self.optimized_params[param_index + 1]
                c = self.optimized_params[param_index + 2]
                mfs.append(lambda x: gbell_mf(x, a, b, c))
                param_index += 3
...
# Load dataset
data = pd.read_csv('Cleveland_heart_attack.csv')
...
mfs_before_training = [
    [lambda x, a=0.5, b=0.1, c=0.8: gbell_mf(x, a, b, c) for _ in range(num_rules)]
...
mfs_after_training = [
    [lambda x: gbell_mf(x, a=np.random.uniform(0, 1), b=np.random.uniform(0.1, 0.5), c=np.random.uniform(0.1, 0.5) ) for _ in range(num_rules)]
    for _ in range(num_features)
]
...
smoking_habits_input = TextInput(value="", title="Smoking Habits (0=Non Smoker, 1=Regular Smoker):")
Family_history_input = TextInput(value="", title="Family History (0=No, 1=Yes):")
...
def submit_callback():
    try:
        ...
        inputs = [
            ...
            ('Smoking Habits', float(smoking_habits_input.value)),
            ('Family History', float(Family_history_input.value))
        ]
        ...
        smoking_habits_display = "Regular Smoker" if int(smoking_habits_input.value) == 1 else "Non Smoker"
        Family_history_display = "Yes" if int(Family_history_input.value) == 1 else "No"
    ...
    # Map prediction to heart attack risk level based on your dataset
    ...
    # Display result
        output_div.text = (
            f"Prediction: Risk Level for Heart Attack - {risk_level}<br>"
            f"Chest Pain Type: {chest_pain_type}<br>"
            f"Fasting Blood Sugar > 120: {fbs_display}<br>"
            f"Resting ECG: {restecg_display}<br>"
            f"Exercise Induced Angina: {exang_display}<br>"
            f"Smoking Habits: {smoking_habits_display}<br>"
            f"Family History: {Family_history_display}"
        )
...
input_row4 = row(smoking_habits_input,Family_history_input)
